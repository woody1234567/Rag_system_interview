...
class TestExperimentsRunner(unittest.TestCase):
    ...
    def test_flatten_summary(self):
        s = {
            ...
            "refusal_f1": 0.3,
            "retrieval": {
                "fusion_k_hit_rate": 0.6,
                "final_k_hit_rate": 0.7,
                "final_context_hit_rate": 0.7,
                "avg_rerank_gain_k": 0.1,
                "pipeline_drop_from_20_to_k": -0.2,
                "rerank_gain": -0.2,
                "avg_rerank_latency_ms": 12.3,
            },
        }
        row = flatten_summary(s)
        self.assertEqual(row["retrieval.final_k_hit_rate"], 0.7)
        self.assertEqual(row["retrieval.avg_rerank_gain_k"], 0.1)

    def test_rank_key_prefers_gain_non_negative(self):
        good = {
            "accuracy_relaxed": 0.4,
            "retrieval.final_k_hit_rate": 0.5,
            "refusal_f1": 0.2,
            "retrieval.avg_rerank_latency_ms": 10,
            "retrieval.avg_rerank_gain_k": 0.0,
        }
        bad = {
            "accuracy_relaxed": 0.9,
            "retrieval.final_k_hit_rate": 0.9,
            "refusal_f1": 0.9,
